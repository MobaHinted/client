...
class Users:
    ...
    def add_user(self, username: Union[str, None] = None, show_popups: bool = True) -> int:
        ...
        def close_popup():
            hinter.imgui.delete_item(item='add-user')
            hinter.imgui.delete_item(item='add-user-input')
            hinter.imgui.delete_item(item='add-user-spacer')
            hinter.imgui.delete_item(item='add-user-button')
        ...
        def submit(sender, data):
            ...
            if sender == 'add-user-button':
                data = hinter.imgui.get_value('add-user-input')
        ...
        if username is None:
            ...
            with hinter.imgui.window(
                ...
                hinter.hinter.imgui.add_spacer(tag='add-user-spacer', height=10)
    ...
    def remove_user(self, username: str = ''):
        # Make sure the username popup can close itself
        def close_popup():
            hinter.hinter.imgui.delete_item(item='remove-user')
            hinter.hinter.imgui.delete_item(item='remove-user-input')
            hinter.hinter.imgui.delete_item(item='remove-user-spacer')
            hinter.hinter.imgui.delete_item(item='remove-user-button')

        # Retry the function, with the provided username
        def submit(sender, data):
            # Correct the data-source if the button was used
            if sender == 'remove-user-button':
                data = hinter.hinter.imgui.get_value('remove-user-input')
        ...
        if username == '':
            width = 165
            height = 100
            with hinter.hinter.imgui.window(
                    label='Remove a User',
                    modal=True,
                    tag='remove-user',
                    width=width,
                ...
                hinter.hinter.imgui.add_input_text(
                ...
                hinter.hinter.imgui.add_spacer(tag='remove-user-spacer', height=10)
                hinter.hinter.imgui.add_button(
